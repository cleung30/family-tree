...
export default function App() {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100vh'}}>
      <header style={{background:'#722F37',color:'#fff',padding:'12px 20px',display:'flex',flexWrap:'wrap',alignItems:'center',gap:12,flexShrink:0}}>
        <div style={{display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
          <div style={{width:36,height:36,borderRadius:8,background:'#faf7f2',color:'#722F37',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,fontWeight:700,flexShrink:0}}>梁</div>
          <div>
        ...
        <div ref={exportWrapRef} style={{position:'relative',flexShrink:0}}>
          <button onClick={() => setExportOpen(o => !o)} title="Export the family tree" style={{...btn,background:'#8a3f56',color:'#fff'}}>⬇ Export</button>
          {exportOpen && (
        ...
        <button onClick={() => setTermsOpen(true)} title="Look up what to call each relative" style={{...btn,background:'#8a3f56',color:'#fff',flexShrink:0}}>称谓 Family Terms</button>
        {isEditor && <button onClick={() => { setSelected(null); setModalMode('add') }} style={{...btn,background:'#fff',color:'#722F37',flexShrink:0}}>+ Add Member</button>}
        {isAdmin && <button onClick={openEditors} style={{...btn,background:'#8a3f56',color:'#fff',flexShrink:0}}>👥 Manage Editors</button>}
        {session ? (
          <button onClick={signOut} title={isEditor ? `Signed in as ${session.user.email}` : `Signed in as ${session.user.email} (view only)`} style={{...btn,background:'#8a3f56',color:'#fff',flexShrink:0}}>{isEditor?'✓ ':''}{session.user.email} · Sign out</button>
        ) : (
          <button onClick={openAuth} style={{...btn,background:'#8a3f56',color:'#fff',flexShrink:0}}>Sign in to edit</button>
        )}
      </header>
      {errorMsg && (
                ...
                {editorsList.map(e => (
                  <div key={e.email} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 10px',background:'#f9fafb',borderRadius:6}}>
                    <div>
                      ...
                      <div style={{fontSize:11,color:'#8a3f56',textTransform:'uppercase',fontWeight:700}}>{e.role}</div>
                    </div>
...
const btn = {padding:'8px 16px',borderRadius:6,border:'none',background:'#722F37',color:'#fff',fontWeight:600,fontSize:14}
