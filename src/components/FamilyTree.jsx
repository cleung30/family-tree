...
export default function FamilyTree({people,relationships,selectedId,onSelect,focusRequest}){
  ...
  return(
          ...
          return(
            <g key={n.id} className="tn" transform={`translate(${n.x},${n.y})`} onClick={()=>onSelect(p)} style={{cursor:'pointer'}}>
              <rect x={-NW/2} y={-NH/2} width={NW} height={NH} rx={8}
                fill={sel?'#4a0404':p.gender==='f'?'#fce7f3':p.gender==='m'?'#dbeafe':'#f3f4f6'}
                stroke={sel?'#4a0404':'#d1d5db'} strokeWidth={sel?2:1}/>
    ...
    <button onClick={()=>{interactedRef.current=false;fitToView()}} title="Reset view" style={{position:'absolute',bottom:16,right:16,width:40,height:40,borderRadius:'50%',border:'1px solid #d1d5db',background:'#fff',color:'#4a0404',fontSize:18,cursor:'pointer',boxShadow:'0 2px 8px rgba(0,0,0,0.15)'}}>⤢</button>
    </>
  )
}
